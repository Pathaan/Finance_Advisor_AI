...
def compare_stocks(symbols):
    ...
    for symbol in symbols:
        try:
            ...
            hist = stock.history(period="3mo")  # Fetch last 3 months data
...
market_analyst = Agent(
    ...
    instructions=[
        ...
        "Calculate percentage change over a 3-month period.",
        "Rank stocks based on their relative performance.",
        "Comment on recent highs/lows and significant price movements."
    ],
...
company_researcher = Agent(
    ...
    instructions=[
        ...
        "Mention current challenges or opportunities mentioned in the news.",
        "Flag any leadership changes, earnings surprises, or legal actions."
    ],
    markdown=True
)
...
stock_strategist = Agent(
    ...
    instructions=[
        ...
        "Assess short-term momentum versus long-term stability."
    ],
    markdown=True
)
...
team_lead = Agent(
    ...
    instructions=[
        ...
        "Flag any high-risk or high-reward opportunities with rationale."
    ],
    markdown=True
)
...
st.set_page_config(page_title="LionVest AI", page_icon="🦁", layout="wide")

# Title and header
st.markdown("""
    <h1 style="text-align: center; color: #4CAF50;">🦁  LionVest AI</h1>
    <h3 style="text-align: center; color: #6c757d;">Generate investment reports with the latest market insights.</h3>
""", unsafe_allow_html=True)

# Sidebar Styling
st.sidebar.markdown("""
    <h2 style="color: #343a40;">Configuration</h2>
    <p style="color: #6c757d;">Enter the stock symbols you want to analyze. The 🦁 will provide you top recommendations.</p>
""", unsafe_allow_html=True)

# Stock symbols input
input_symbols = st.sidebar.text_input("Enter Stock Symbols (separated by commas)", "HDFCBANK.NS, ICICIBANK.NS, SBIN.NS, AXISBANK.NS,")
...
if st.sidebar.button("🦁 we provide best recommendation"):
    ...
    # elif not api_key:
        ...
        st.info("🦁 report provides detailed insights, including market performance, company analysis, and investment recommendations.")

        # Interactive Stock Performance Chart
        st.markdown("###  Stock Performance (3-Months)")
        stock_data = yf.download(stocks_symbols, period="3mo")['Close']

        fig = go.Figure()
        for symbol in stocks_symbols:
            fig.add_trace(go.Scatter(x=stock_data.index, y=stock_data[symbol], mode='lines', name=symbol))

        fig.update_layout(title="Stock Performance Over the Last 3 Months",
                          xaxis_title="Date",
